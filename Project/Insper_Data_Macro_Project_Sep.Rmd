...
Thinking about debt tolerance, Reinhart, Rogoff and Savastano (2003) introduce the concept of debt intolerance and analyze how emerging markets find it difficult to face high levels of debt, while advanced countries face this issue more easily.  Later, Caṭo and Kapur (2006) discussed the macroeconomic determinants of a country's volatility and how it impacts the spread rate it has to pay.
...
##The level of indebtedness in the world has changed in recent years
```{r , echo=FALSE, message=FALSE}
dataset_total %>% 
  group_by(year) %>% 
  summarise(debt_to_GDP_mean = mean(debt_to_GDP_)) %>% 
ggplot(aes(x = year, y = debt_to_GDP_mean, color = "develop" )) +
    geom_point() +
    labs(x = "Year", y = "Debt to GDP ratio (%)", title = "                          Average debt to GDP ratio for 47 countries") +
    theme_bw()
```
```{r,  echo=FALSE, message=FALSE}
...
graf_GDP_debt_2005 <- dataset_total %>% 
  filter(year==2005) %>% 
  ggplot(aes(x = GDP_per_cap_cur_USD, y = debt_to_GDP_, color=country)) + 
  labs(x = "GDP per capita", y = "Debt to GDP ratio (%)", title = "                    GDP per capita x Debt to GDP ratio (2005)")+
...
graf_GDP_debt_2015 <- dataset_total %>% 
  filter(year==2015) %>% 
  ggplot(aes(x = GDP_per_cap_cte, y = debt_to_GDP_, color=country)) + 
  labs(x = "GDP per capita", y = "Debt to GDP ratio (%)", title = "                    GDP per capita x Debt to GDP ratio (2015)")+
  geom_point(show.legend = FALSE) +
  theme_bw()

graf_GDP_debt_2015

graf_GDP_debt_2019 <- dataset_total %>% 
  filter(year==2019) %>% 
  ggplot(aes(x = GDP_per_cap_cte, y = debt_to_GDP_, color=country)) + 
  labs(x = "GDP per capita", y = "Debt to GDP ratio (%)", title = "                    GDP per capita x Debt to GDP ratio (2019)")+
  geom_point(show.legend = FALSE) +
  theme_bw()

graf_GDP_debt_2019

ggplotly(graf_GDP_debt_2005, tooltip = c("country", "debt_to_GDP"))
